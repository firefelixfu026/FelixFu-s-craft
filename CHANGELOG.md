# 版本日志

## v2.7.4 - 首页秒级计时热修

- 修复首页 `homeNow is not defined`，秒级上线计时状态移动到首页组件内部。
- 首页恢复正常渲染，继续保留上线时间每秒自动刷新。

## v2.7.3 - 首页上线时间精确到秒

- 首页“这个站点正在发生什么”里的上线信息从整数天改为实时天、小时、分钟、秒。
- 前端每秒刷新一次上线计时，不需要手动刷新页面。
- 状态卡补充长文本换行和手机端字号处理，避免秒级计时在窄屏上撑破布局。

## v2.7.2 - 运维页加载稳定性

- 运维面板改为随主入口一起加载，避免部署后手机浏览器旧缓存继续请求过期的动态分包文件。
- 保留运维页局部错误保护，后续如果真实健康检查接口异常，后台仍能回到总览继续使用。
- 这次修复主要针对“点运维页打不开/白屏/提示页面卡住”的部署后偶发现象。

## v2.7.1 - 移动端布局整理

- 移动端侧栏改为紧凑的单行横向滚动导航，避免头像区和图标导航占据过多高度。
- 横屏移动端让品牌区和导航并排显示，减少正文被顶部导航压住的情况。
- 暑期计划表格在手机宽度下改为卡片流展示，每个字段显示自己的标签。
- 移动端计划表卡片补齐夜间模式颜色，避免深色主题下出现白块。

## v2.7 - 主页故事线和评论体验

- 首页新增“故事线”模块，把课程笔记、工程闭环和 AI 工作流串成更像个人作品集的叙事。
- 首页新增“作品墙”和“当前探索”，突出个人博客工作台、AI 写作学习助手、运维和备份面板。
- 文章评论区新增公开评论数、最近互动、我的状态摘要，并支持站长、我、待审核徽章。
- 登录用户现在可以看到自己待审核的评论，避免提交后评论突然消失造成误解。

## v2.6.3 - 夜间模式文字对比度修复

- 修复备份中心统计卡在夜间模式下标签和值过暗的问题，`release-metric` 内部文字改为跟随主题变量。
- 补齐版本指标、后台说明文字、暑期计划标题说明和日期选择器的深色主题文字颜色。
- 继续收拢硬编码浅色灰字，减少后台各板块在夜间模式中出现“看不清”的残留区域。

## v2.6.2 - 后台夜间模式补全

- 补齐后台文章库、图片管理、评论管理、安全页、备份中心和微信群 bot 语料工作台的夜间模式样式。
- 后台列表卡片、图片资源卡、说明卡、空状态、上传按钮和普通操作按钮统一使用深色主题变量。
- 删除按钮、文章状态徽章和待审徽章新增深色主题配色，避免浅色背景在夜间模式中过亮。
- 修复安全/备份说明卡和暂无内容提示仍显示浅色底的问题。

## v2.6.1 - 可用性、安全和性能打磨

- 文章中心搜索增强，支持普通关键词高亮，并支持 `tag:`、`category:` 和 `month:` 高级筛选。
- 首页新增站点实时状态卡片，展示上线天数、公开文章数、阅读记录、评论互动和最近更新。
- 评论管理新增“通过当前筛选”批量审核操作，减少多条待审评论逐条处理的成本。
- 登录流程新增前端本地锁定和后端 IP + 邮箱维度失败限流，降低撞库和暴力尝试风险。
- 移动端布局补充首页状态卡、最近更新条和深色模式高亮样式，减少窄屏挤压和低对比度问题。
- 前端生产构建拆分 React、图标和 KaTeX 代码块，降低主包体积压力。
- `.env.example` 新增 `LOGIN_FAILURE_LIMIT` 和 `LOGIN_LOCK_SECONDS`，方便线上调整登录防护阈值。

## v2.6.0 - 后台工具扩展和 AI 可视化配置

- 后台 AI 页新增保存、测试、停用和删除配置，管理员可直接在前端维护真实模型参数。
- 后端新增持久化 AI 设置文件，保存后的写作工作台和后台 AI 写作会优先使用真实模型，停用后回到本地模板。
- 写文章页新增写作模板库和草稿版本历史，可快速套用项目复盘、论文阅读、漏洞分析和学习周报结构。
- 后台新增备份中心、微信群 bot 语料工作台和学习助手三个独立页面。
- 安全页新增前端错误日志面板，全局卡住页会把错误写入浏览器本地日志。
- 运维页新增容器状态面板，后端会尝试读取 `docker compose ps` 并在不可用时给出清晰提示。

## v2.1.4 - 账号收藏和管理员评论审核开关

- 后端新增 `ADMIN_COMMENTS_REQUIRE_APPROVAL` 环境变量，默认 `false`，管理员评论默认直接公开，也可切换为需要审核。
- 账号活动接口新增 `favoriteArticles`，把收藏文章从普通互动记录中单独整理出来。
- 账号中心新增“我的收藏”区域，可查看收藏文章并跳转到文章详情。
- README 补充评论审核开关、收藏文章和账号中心说明。

## v2.1.3 - 登录注册和 AI 权限收口

- 新增普通邮箱注册流程，普通用户可注册读者账号后评论、点赞、收藏、点踩和使用“？”反馈。
- 管理员初始化改为独立接口 `/api/auth/admin/register`，继续要求 `ADMIN_SETUP_TOKEN`，避免和普通注册混在一起。
- 游客导航收口为文章、游戏和登录，降低无登录状态下的页面复杂度。
- 普通用户不再显示 AI 入口，AI 工作台和 AI 写作辅助只保留给管理员使用。
- README 同步说明普通注册、管理员初始化和 AI 权限边界。

## v2.1.2 - 仓库文档中文化整理

- 根目录 `README.md` 改为中文项目介绍，方便验收时快速了解项目目标、功能、技术栈和部署方式。
- `docs/README.md` 改为中文文档导航，统一说明正式文档和归档文档的用途。
- `docs/guides/` 下正式文档改为中文文件名，包括部署、域名、数据库、备份、GitHub 登录、自动部署和服务器运维说明。
- 保留 GitHub 常见约定文件名，例如 `README.md` 和 `CHANGELOG.md`，但内容优先使用中文。

## v2.1.1 - AI Codex 中转接口适配

- 后端新增 `AI_API_STYLE` 配置，支持 `openai` 和 `codex` 两种真实模型接口类型。
- Codex 模式会直接请求中转站提供的完整地址，不再自动拼接 `/chat/completions`。
- AI 返回内容解析器兼容 `choices`、`output_text`、`content`、`result`、`output[].content[].text` 和 SSE `data:` 等常见形态。
- 当中转站返回网页 HTML 时，后台会明确提示 Base URL 不是完整 API 地址，避免误判为模型输出。
- 后台 AI 设置和测试面板新增“接口类型”选择，可直接测试 AICodeMirror 等 Codex 中转接口。
- `.env.example`、`docker-compose.yml` 和部署文档补充 Codex 中转配置说明。

## v2.1.0 - 项目验收收尾版

- 后台新增站点统计面板，展示总阅读、用户、评论、待审评论、草稿、热门文章和分类分布。
- 后台新增 AI 设置和测试面板，可读取服务器 AI 配置状态，并使用一次性输入测试真实模型连通性。
- AI 写作历史改为后端持久化记录，刷新后仍可在管理后台查看最近生成任务。
- 文章新增分类和置顶字段，文章列表支持分类筛选，并保留月份归档和标签筛选。
- 评论系统新增回复目标和账号级频率限制，评论继续走审核流程。
- 新增账号中心，登录用户可查看自己的评论、互动和收藏记录。
- 后端新增账号活动、后台统计、AI 设置、AI 测试和 AI 历史接口。
- 补充 API Key 安全说明：密钥只放环境变量或一次性测试框，不写入仓库。

## v2.0.0 - 发布候选完整闭环

- 首页阶段升级为 v2.0.0，明确展示博客已经完成文章、评论、图片、登录、部署和 AI 写作辅助闭环。
- 管理后台新增“2.0 发布状态”面板，汇总已发布文章、草稿、待审评论、图片资源和 AI 生成记录。
- AI 写作辅助新增最近生成历史，记录任务类型、插入方式、结果来源和生成时间。
- AI 写作历史支持撤回最近一次生成结果，减少润色、续写或大纲插入后的误操作成本。
- 发布清单文档新增 v2.0.0 review 项，方便在正式进入后续 AI 模块深化前做整体验收。

## v1.7.5 - AI 写作结果可控插入

- 管理后台 AI 写作辅助新增结果插入方式：追加、插入光标处、替换选中内容。
- “替换”模式会优先替换选中文本；未选中时会自动退化为插入光标处，避免误删正文。
- AI 生成完成后的后台提示会明确说明结果已追加、插入或替换到正文。
- AI 工具条新增分段式插入模式控件，移动端可换行显示。
- 首页阶段和 AI 能力文案同步更新为 v1.7.5。

## v1.7.4 - 后台 AI 写作辅助

- 后端新增 `/api/ai/editor` 管理员接口，支持润色、续写和生成大纲三种写作任务。
- 写作后台正文区域新增“AI 润色”“AI 续写”“AI 大纲”按钮，可基于选中文字或整篇正文生成辅助内容。
- AI 写作结果会追加到正文末尾，并标注真实模型、本地占位或回退结果来源。
- 真实模型未配置或调用失败时继续使用本地回退内容，避免影响后台写作流程。
- 首页阶段和 AI 能力文案同步更新为 v1.7.4。

## v1.7.3 - AI 结果进入写作后台

- AI 工作台结果卡新增“填入表单”按钮，可将候选标题、摘要、标签和写作提示带入管理后台文章表单。
- 管理员使用“填入表单”后会自动跳转到写作后台，并将文章状态设为草稿，避免误发布。
- 如果当前表单已有内容，AI 候选会追加到正文下方，并合并标签，不会直接覆盖已有草稿。
- 未登录管理员时点击“填入表单”会跳转登录页并给出明确提示。
- 首页阶段和 AI 能力文案同步更新为 v1.7.3。

## v1.7.2 - 真实 AI 模型调用

- AI 工作台支持调用 OpenAI 兼容的 `/chat/completions` 接口，可通过 `AI_PROVIDER_NAME`、`AI_BASE_URL`、`AI_MODEL`、`AI_API_KEY` 配置真实模型。
- 后端会要求模型返回结构化 JSON，并清洗为标题、摘要、标签和下一步动作四类结果。
- 真实模型调用失败时自动回退到本地候选内容，避免 AI 配置或网络问题影响页面可用性。
- AI 工作台结果区新增来源提示，可区分真实模型、本地占位和调用失败后的回退状态。
- Docker Compose 和 `.env.example` 新增 `AI_REQUEST_TIMEOUT`，用于控制真实模型请求等待时间。

## v1.7.1 - 首页刷新和 AI 配置状态

- 首页文案和指标更新到当前阶段，展示文章系统、AI 工作台、小游戏和写作后台四个核心能力。
- 首页新增快速入口按钮，可直接进入文章页和 AI 工作台。
- 后端新增 `/api/ai/status`，用于展示当前 AI Provider、模型和真实模型配置状态。
- AI 工作台新增配置状态提示，区分“本地占位模式”和“真实模型已配置”。
- Docker Compose 新增 `AI_PROVIDER_NAME`、`AI_BASE_URL`、`AI_MODEL` 和 `AI_API_KEY` 环境变量占位。

## v1.7.0 - AI 工作台骨架

- 后端新增 `/api/ai/workbench` 占位接口，支持文章灵感、摘要生成和标题优化三种模式。
- AI 工作台暂时使用本地确定性生成逻辑，不消耗真实模型额度。
- 前端 AI 页面升级为可交互工作台，支持输入主题、正文/背景材料、语气和标签。
- 生成结果以候选卡片展示，可一键复制标题、摘要和标签。
- AI 页面保留技术新闻和已有文章作为参考素材，为后续自动内容流做铺垫。

## v1.6.0 - 写作后台增强包

- 后端新增管理员图片资源管理接口：可列出 `/uploads` 下的图片，并删除不再使用的图片。
- 管理后台新增“图片管理”面板，支持预览、复制图片地址、插入正文和删除图片。
- 文章正文编辑区新增 Markdown 工具栏，支持标题、加粗、列表、引用、代码块和 LaTeX 公式快捷插入。
- 文章表单新增本地草稿自动保存，刷新页面或误关浏览器后可恢复草稿。
- 发布文章或清空表单后会自动清理本地草稿，避免旧草稿误覆盖新内容。
- 这一版完成基础写作后台闭环，下一阶段进入 AI 模块开发。

## v1.5.11 - 正文插图上传

- 管理后台文章正文区域新增“上传正文图片”按钮。
- 上传成功后会自动生成 Markdown 图片语法 `![文件名](/uploads/...)`，并插入到当前光标位置。
- 如果正文没有光标位置，会把图片追加到正文末尾。
- 封面上传和正文插图上传共用同一套图片类型、大小和错误提示校验。

## v1.5.10 - 图片上传失败提示热修

- 前端上传前会检查图片类型和大小，只允许 JPG、PNG、WebP、GIF 和 SVG，单文件不超过 5 MB。
- 图片上传失败时会显示更明确的原因，包括 HTTP 状态码和 Nginx 413 上传上限提示。
- 云服务器部署文档和运维文档补充宿主机 Nginx `client_max_body_size 6m;` 配置，避免大图在进入容器前被拦截。

## v1.5.9 - 后台图片上传

- 后端新增管理员图片上传接口 `/api/admin/uploads/images`，只有管理员登录后才能使用。
- 支持 JPG、PNG、WebP、GIF 和 SVG 图片，单文件上限 5 MB。
- 上传文件会保存到后端 `/uploads` 目录，并通过站内 `/uploads/...` 地址访问。
- Docker Compose 新增 `uploads_data` 持久卷，避免容器重建后上传图片丢失。
- 前端 Nginx 新增 `/uploads/` 代理，并把请求体上限提高到 6 MB。
- 管理后台文章表单新增“上传封面图”，上传成功后会自动填入封面图地址并显示预览。
- 已执行 `python -m compileall backend\app`、`npm run build` 和 `docker compose config` 验证通过。

## v1.5.8 - 文章封面图字段和展示

- 后端 `articles` 表新增 `cover_url` 字段，并通过启动时轻量迁移兼容已有数据库。
- 文章创建、编辑和公开接口新增 `coverUrl`，管理员可以为文章配置站内路径或外部图片地址。
- 前端首页最近文章、文章列表和文章详情页支持展示文章封面图。
- 管理后台文章表单新增“封面图地址”输入和即时预览。
- 旧笔记 `Git` 会默认使用 `/articles/git-workflow.svg` 作为封面；已有数据库文章如果封面为空会自动补齐。
- 已执行 `python -m compileall backend\app` 和 `npm run build` 验证通过。

## v1.5.7 - 文章图片资源本地化修复

- 将旧笔记 `Git` 中失效的飞书临时图片链接替换为本站本地资源 `/articles/git-workflow.svg`。
- 新增本地 Git 工作流示意图，避免外链过期、跨域或鉴权导致文章图片无法加载。
- 后端种子逻辑会在启动时自动修复数据库中已存在旧文章的过期图片链接，不需要手动重发文章。
- 前端图片组件新增加载失败提示，后续遇到失效图片时会显示明确原因而不是只显示破图图标。
- 已执行 `python -m compileall backend\app` 和 `npm run build` 验证通过。

## v1.5.6 - Markdown 格式兼容增强

- Markdown 渲染器新增 1-6 级标题支持，旧笔记中的 `####`、`#####` 不再显示为原始文本。
- 新增 `---`、`***`、`___` 分隔线渲染。
- 新增编号列表渲染，旧笔记中的连续 `1.`、`2.`、`3.` 会显示为有序列表。
- 新增基础 Markdown 表格渲染，支持表头、对齐标记和横向滚动。
- 图片识别逻辑更宽容，兼容带标题、尖括号包裹和较长查询参数的图片地址。
- 已执行 `npm run build` 验证前端构建通过。

## v1.5.5 - Markdown 图片阅读体验

- 文章正文 Markdown 渲染器新增独立图片语法支持：`![说明](图片地址)` 会渲染为居中的图片块。
- 行内 Markdown 新增图片语法支持，可在段落文字中插入小图。
- 图片统一增加最大宽度、圆角、边框和懒加载，避免旧笔记外链图片撑破文章布局。
- 长图和行内图都增加尺寸约束，手机端阅读时不会横向溢出。
- 已执行 `npm run build` 验证前端构建通过。

## v1.5.4 - 旧版笔记迁移

- 从旧版 GitHub Pages 笔记站 `FelixFu-no-website` 迁移 6 篇 Markdown 笔记：Git、Vibe Coding、计算机网络、后端技术指南、前端三件套、React 基础与进阶。
- 后端新增 `legacy_notes.py`，将旧笔记作为可自动补齐的文章种子数据。
- `seed_database` 改为逐篇检查缺失文章，不再因为数据库已有文章就跳过全部种子导入。
- 部署后服务启动会自动插入缺失旧笔记，不会覆盖现有文章、评论或互动数据。
- 已执行 `python -m compileall backend\app` 和 `npm run build` 验证通过。

## v1.5.3 - 文章页白屏热修复

- 修复文章页组件漏传月份归档参数导致进入“文章”页面后前端运行时崩溃、页面空白的问题。
- 给文章页组件的文章列表、标签和月份归档参数增加安全默认值，降低后续类似漏传导致白屏的风险。
- 已执行 `npm run build` 验证前端构建通过。

## v1.5.2 - 阅读次数和热门文章排行

- 后端 `articles` 表新增 `view_count` 字段，历史文章默认从 0 开始累计。
- 游客或普通用户打开文章详情时会自动增加一次阅读次数，管理员查看不计入阅读次数。
- 文章列表和文章详情顶部新增阅读次数展示。
- 文章中心新增“热门文章”模块，按阅读次数展示前 5 篇文章，并可直接进入文章详情。
- 已执行 `python -m compileall backend\app` 和 `npm run build` 验证通过。

## v1.5.1 - 评论审核流程

- 后端 `comments` 表新增 `status` 字段，历史评论默认保持已通过，新评论默认进入待审核。
- 公开文章详情只显示已通过评论，待审核评论不会直接暴露给游客。
- 管理后台评论列表新增状态筛选和状态徽章，可按全部、待审核、已通过查看评论。
- 管理员可以在评论管理里一键通过待审核评论，也可以继续删除不合适的评论。
- 评论提交成功后提示“审核通过后会公开显示”，减少用户误解。
- 已执行 `python -m compileall backend\app` 和 `npm run build` 验证通过。

## v1.5.0 - 文章草稿和发布状态

- 后端 `articles` 表新增 `status` 字段，老文章默认保持已发布。
- 管理员发布或编辑文章时可选择“已发布”或“草稿”。
- 公开文章列表、文章详情、评论和互动接口会隐藏草稿；管理员登录后仍可查看和编辑全部文章。
- 管理后台文章列表新增状态徽章，便于区分草稿和已发布文章。
- 已执行 `python -m compileall backend\app` 和 `npm run build` 验证通过。

## v1.4.2 - 移动端文章阅读体验优化

- 优化窄屏文章阅读布局，文章列表在手机上稳定单列显示。
- 文章详情页标题、正文、目录和公式区域在手机上收紧留白。
- 标签和月份归档筛选在窄屏下支持横向滚动，避免按钮挤压换行过长。
- 手机端互动按钮改为两列布局，阅读、返回和评论输入区域更易点击。
- 已执行 `npm run build` 验证前端构建通过。

## v1.4.1 - 文章详情目录和标题锚点

- 文章详情页会根据 Markdown 标题自动生成文章目录。
- 正文中的标题自动获得锚点，点击目录项可跳转到对应段落。
- 目录支持多级缩进，方便阅读长篇学习笔记和项目记录。
- 已执行 `npm run build` 验证前端构建通过。

## v1.4.0 - 文章月份归档和组合筛选

- 文章页新增月份归档筛选，按文章日期自动生成月份入口。
- 每个月份入口显示该月文章数量。
- 搜索、标签筛选和月份归档可以组合使用。
- 无匹配文章时显示空状态提示。
- 已执行 `npm run build` 验证前端构建通过。

## v1.3.1 - LaTeX 公式渲染和文章预览

- 接入 KaTeX，文章详情页支持将 LaTeX 公式渲染为真正的数学排版。
- 行内公式支持 `$...$` 和 `\(...\)`。
- 块级公式支持 `$$...$$` 和 `\[...\]`，并支持多行公式块。
- 管理后台正文输入框下新增 Markdown / LaTeX 实时预览，发布前可以检查正文、代码块和公式显示效果。
- 已执行 `npm run build` 验证前端构建通过。

## v1.3.0 - 文章详情页和基础 Markdown 阅读

- 文章列表改为摘要卡片，点击“阅读全文”进入详情页。
- 详情页集中展示正文、互动和评论，并支持返回文章列表。
- 新增无依赖 Markdown 渲染：标题、段落、列表、引用、代码块、行内代码、粗体/斜体、公式块和行内公式的基础显示。
- 已执行 `npm run build` 验证前端构建通过。

## v1.2.6 - 评论体验修补

- 评论管理新增按文章筛选和按作者筛选。
- 删除后台评论后保留当前分页位置，不再强制回到第一页。
- 前台评论发布成功后显示“评论发布成功”，失败时显示明确错误提示。
- 前台评论输入区显示将以当前登录账号身份发表评论。
- 已执行 `npm run build` 验证前端构建通过。
## v1.2.5 - 后台评论管理分页和限宽

- 后台评论管理改为固定每页 5 条评论，不再一次性展示全部评论。
- 后台评论列表新增上一页/下一页分页控件。
- 修复后台分页复用显示长度算法导致部分页面只显示 1 条短评论的问题。
- 管理后台评论行增加最大宽度和强制断行，连续数字、连续英文和长链接不会再撑破页面。
- 已执行 `npm run build` 验证前端构建通过。
## v1.2.4 - 评论权限和长文本限宽修复

- 评论接口改为必须登录，游客不能再直接发表评论。
- 评论作者名由后端从当前登录账号读取，GitHub 登录后会使用 GitHub 显示名或登录名。
- 未登录点击点赞、收藏、点踩、“？”或发布评论时，只显示站内提示，不再直接跳转到登录页。
- 修复连续数字或连续英文评论不会自动换行、把文章卡片横向撑开的显示问题。
- 评论展示区、评论输入区和后台评论文本都增加最大宽度与强制断行规则。
- 已执行 `npm run build` 验证前端构建通过。
## v1.2.3 - 账号级互动和评论分页

- 新增 `user_reactions` 表，点赞、收藏、点踩和“？”互动都按登录账号记录。
- 游客点击互动按钮会被引导到登录页，不能匿名累计互动数。
- 同一账号对同一篇文章的同一种互动只能保留一条记录，刷新网页不会重复增加计数。
- 启动时执行一次互动数据清理：清空旧的匿名点赞、收藏、点踩计数，并从账号记录重新同步计数。
- 评论输入新增 300 字上限，评论列表按显示长度分页，每页最多约 5 条短评论的展示量。
- 已执行 `python -m compileall backend\app`、`npm run build` 和 `docker compose config` 验证通过。
## v1.2.2 - 游客隐藏管理入口

- 前端导航改为按身份显示：游客只看到首页、文章、AI、游戏和登录。
- 只有管理员登录后才显示“管理”入口。
- 未登录或非管理员状态进入后台视图时会自动回到首页，避免向访客暴露后台登录页面。
## v1.2.1 - 管理员初始化安全加固

- 管理员初始化接口新增 `ADMIN_SETUP_TOKEN` 校验，避免公开站点被陌生人抢先初始化管理员。
- 如果服务器未配置 `ADMIN_SETUP_TOKEN`，后端会默认拒绝初始化管理员，现有管理员登录不受影响。
- 登录页“初始化管理员”模式新增“初始化密钥”输入框。
- `.env.example`、README、Docker Compose 和云部署文档已补充初始化密钥说明。
## v1.2.0 - 管理后台评论管理

- 后端新增 `GET /api/admin/comments`，管理员可集中查看全站评论。
- 后端新增 `DELETE /api/admin/comments/{comment_id}`，管理员可删除不合适的评论。
- 管理后台新增“评论管理”区域，支持查看评论所属文章、评论人、发布时间和内容。
- 管理后台支持刷新评论列表和删除单条评论，删除后自动刷新文章与评论状态。
- 已执行 `python -m compileall backend\app` 和 `npm run build` 验证通过。
## v1.1.2 - 个人头像上线验证

- 新增 `frontend/public/avatar.jpg`，作为网站个人头像资源。
- 侧边栏品牌位从 `FJ` 文本标识改为真实头像图片。
- 首页个人展示图同步改为头像，便于线上确认自动部署效果。
- 已执行 `npm run build` 验证前端构建通过。
- 本次提交用于验证 GitHub Actions 自动部署链路是否会在 push 后自动运行。
## v1.1.1 - 自动部署首次运行修复

- 修复 Deploy workflow 首次运行时，服务器尚未拉取 `scripts/deploy-production.sh` 导致 `chmod` 找不到文件的问题。
- Deploy workflow 现在会先在服务器执行 `git pull --ff-only`，再运行部署脚本。
- 当前失败点说明：SSH 和 GitHub Secrets 已验证通过，问题只在部署脚本执行顺序。
## v1.1.0 - GitHub Actions 自动部署

- 新增 `.github/workflows/deploy.yml`，在 CI 成功后自动 SSH 到阿里云服务器执行部署。
- 新增 `scripts/deploy-production.sh`，统一服务器手动部署和 Actions 自动部署流程。
- 新增 `GitHub自动部署指南.md`，记录 SSH key、GitHub Secrets、手动触发和故障回退说明。
- 更新 `README.md` 和 `NEXT_STEPS.md`，补充自动部署入口和下一步配置清单。
## v1.0.2 - 备份验证和端口安全收紧

- 线上数据库备份已验证成功，生成 `felix_blog_20260704_023709.sql.gz`。
- HTTPS 证书自动续期 dry-run 已验证成功。
- `docker compose ps` 显示 backend、frontend、postgres 均正常运行。
- `docker-compose.yml` 的 `5432`、`8000`、`8080` 改为只绑定 `127.0.0.1`，避免公网绕过 Nginx 直连数据库、后端或前端容器。
- 已执行 `docker compose config` 验证配置合法。
## v1.0.1 - 服务器日常运维文档

- 新增 `服务器运维手册.md`，记录 SSH 登录、服务状态、日志、更新、重启、Nginx、HTTPS、备份、磁盘检查和常见故障排查。
- 更新 `README.md`，增加上线后运维入口。
- 更新 `NEXT_STEPS.md`，把后续重点推进到备份验证、运维检查和自动部署。
## v1.0.0 - 数据库备份和恢复

- 新增 `数据库备份和恢复.md`，记录线上 PostgreSQL 手动备份、恢复、下载备份和自动备份流程。
- 新增 `scripts/backup-postgres.sh`，用于服务器上生成压缩数据库备份并清理旧备份。
- 新增 `scripts/restore-postgres.sh`，用于从 `.sql.gz` 备份恢复数据库，并在恢复前自动做安全备份。
- 更新 `.gitignore`，忽略本地生成的文档和代码备份目录。
## v0.9.4 - 文章正文展示修复

- 修复文章页只显示摘要、不显示正文的问题。
- 前端文章卡片新增正文区域，渲染 `article.content`。
- 正文区域保留换行并兼容纯文本和 Markdown 原文展示。
- 已执行 `npm run build` 验证前端构建通过。
## v0.9.3 - 正式域名 HTTPS 和 GitHub 登录成功

- 正式站点 `https://www.felixfu.xyz` 已可访问。
- HTTPS 已配置完成。
- GitHub OAuth 登录已验证成功。
- v0.9.0 云服务器部署目标基本完成，下一阶段进入上线后检查、内容发布、备份和自动化增强。
- 更新 `README.md`、`NEXT_STEPS.md`、`云服务器部署指南.md` 和 `GitHub登录配置指南.md`。
## v0.9.2 - GitHub OAuth 环境变量排查

- 记录 GitHub 登录失败原因：服务器 `.env` 中 `GITHUB_CLIENT_ID` 未配置。
- 确认 GitHub OAuth App 的正式域名 URL 配置方向正确。
- 更新 `云服务器部署指南.md`、`NEXT_STEPS.md` 和 `GitHub登录配置指南.md`，补充线上 `.env` 配置和重启命令。
## v0.9.1 - 域名解析正常，准备 HTTPS

- 记录域名 `www.felixfu.xyz` 的 A 记录已显示解析正常。
- 当前 HTTPS 仍未设置，下一步是在服务器配置 Nginx 正式域名和 Certbot 证书。
- 更新 `README.md`、`NEXT_STEPS.md`、`云服务器和域名准备.md` 和 `云服务器部署指南.md`，同步域名阶段状态。
## v0.9.0 - 阿里云公网 IP 部署成功

- 阿里云中国香港服务器公网 IP `47.242.176.227` 已可访问博客。
- Docker Compose 已在服务器启动前端、后端和 PostgreSQL。
- Nginx 已完成 80 端口反向代理到前端容器。
- 当前仍等待域名审批，后续需要配置 DNS、HTTPS 和 GitHub OAuth 正式回调。
- 更新 `README.md`、`NEXT_STEPS.md` 和 `云服务器部署指南.md`，记录公网 IP 阶段部署成功。
## v0.8.5 - Docker 环境验证通过

- 记录阿里云服务器已完成 Docker 安装。
- 记录版本：`Docker version 29.0.1`、`Docker Compose version v5.3.0`。
- 下一步调整为：拉取 GitHub 仓库、配置线上 `.env`、启动 Docker Compose，并配置 Nginx 反向代理到公网 IP。
## v0.8.4 - SSH 和防火墙状态确认

- 记录阿里云服务器 SSH 用户名为 `admin`。
- 记录服务器密码已设置，后续建议切换为 SSH 密钥。
- 记录阿里云轻量服务器防火墙已开放 `22`、`80`、`443`。
- 更新下一步为：在服务器中安装 Docker、拉取仓库并启动 Docker Compose。
## v0.8.3 - 阿里云服务器信息记录

- 记录已购买的阿里云中国香港服务器信息：Ubuntu 22.04、2 vCPU、2 GiB、40 GiB、公网 IP `47.242.176.227`。
- 更新 `云服务器部署指南.md`，明确域名审批前先使用公网 IP 部署和验证。
- 更新 `NEXT_STEPS.md`，把下一步调整为 SSH 连接服务器并部署 Docker Compose。
- 更新 `README.md` 和 `云服务器和域名准备.md`，同步当前服务器和域名审批状态。
## v0.8.2 - 阿里云部署路线确认

- 明确 v0.9.0 云部署优先使用阿里云。
- 更新 `云服务器和域名准备.md`，新增阿里云购买建议、地域选择、安全组端口和备案路线。
- 更新 `云服务器部署指南.md`，新增阿里云购买、控制台、安全组、DNS 配置说明。
- 更新 `NEXT_STEPS.md`，把下一阶段调整为 v0.9.0 阿里云服务器部署。
- 更新 `README.md`，在下一步中明确阿里云优先路线。
## v0.8.1 - 启动和云部署文档整理

- 新增 `云服务器部署指南.md`，记录从服务器初始化到 Docker Compose、Nginx、DNS、HTTPS 和 GitHub OAuth 上线配置的完整流程。
- 更新 `README.md`，补充本地 Docker Compose 快速启动入口和 v0.9.0 云部署路线。
- 更新 `NEXT_STEPS.md`，明确当前阶段是 v0.8.1 文档整理版，下一步是 v0.9.0 云服务器部署。
- 更新 `本地和服务器启动指南.md`，增加快速启动清单和云服务器部署关系说明。
- 更新 `云服务器和域名准备.md`，增加部署执行入口并链接到云部署指南。
## v0.8.0 - GitHub Actions 自动测试

- 新增 `.github/workflows/ci.yml`。
- 新增 `云服务器和域名准备.md`，记录云服务器、域名、备案、DNS 和 HTTPS 准备事项。
- CI 在 push 到 `main`、PR 到 `main`、手动触发时运行。
- 新增 `Backend` job：安装 Python 3.12、安装后端依赖、编译后端代码、导入 FastAPI app。
- 新增 `Frontend` job：安装 Node 22、执行 `npm ci`、执行 `npm run build`。
- 新增 `Compose` job：检查 Docker Compose 版本并执行 `docker compose config`。
- README 新增 CI badge。
- 更新 README、下一步计划、数据库说明、Docker Compose 文档和代码分析文档。

## v0.7.0 - Docker Compose 一键启动

- 新增 `backend/Dockerfile`，容器内运行 FastAPI 后端。
- 新增 `frontend/Dockerfile`，使用 Node 构建前端，再由 Nginx 服务静态文件。
- 新增 `frontend/nginx.conf`，把 `/api` 反向代理到后端容器。
- 扩展 `docker-compose.yml`，支持 `postgres`、`backend`、`frontend` 三服务一键启动。
- 新增 Docker 健康检查，后端依赖数据库健康后启动，前端依赖后端健康后启动。
- 新增 `.dockerignore`、`backend/.dockerignore`、`frontend/.dockerignore`，减少构建上下文。
- 新增 `本地和服务器启动指南.md`，记录启动、停止、验证、Clash 代理和 OAuth 配置方式。
- README 和下一步计划已更新为 Docker Compose 优先。

## v0.6.0 - GitHub OAuth 登录

- 新增 GitHub OAuth 登录跳转和回调接口。
- 新增 GitHub code 换取 access token、读取用户资料和主邮箱的后端模块。
- OAuth state 使用 HMAC 签名并设置有效期。
- GitHub 用户登录后会写入 `users.github_id`、`avatar_url` 和显示名称。
- 支持通过 GitHub 邮箱绑定已有账号。
- 支持 `GITHUB_ADMIN_LOGINS` 和 `GITHUB_ADMIN_EMAILS` 指定 GitHub 管理员。
- 前端登录页新增“使用 GitHub 登录”按钮。
- 前端支持解析 OAuth 回调 token，登录后自动进入管理页。
- 新增 `GitHub登录配置指南.md`，记录 GitHub OAuth App 创建和本地环境变量配置步骤。
- 更新 README、数据库说明、下一步计划和代码分析文档。

## v0.5.0 - 登录鉴权和后台保护

- 新增后端鉴权工具：PBKDF2 密码哈希、HS256 token 签发和校验。
- 新增认证接口：初始化管理员、登录、获取当前用户。
- `POST/PUT/DELETE /api/admin/articles` 已要求管理员 Bearer token。
- 前端新增“登录”页，支持初始化管理员和邮箱密码登录。
- 前端管理页已加登录保护，未登录时显示登录表单。
- 登录状态保存到浏览器本地，刷新后通过 `/api/auth/me` 校验。
- 本地数据库启动时会自动给旧版 `users` 表补充 `password_hash` 字段。
- 修复小游戏仓库中局内打开规则页后返回开始页的问题，相关提交已推送到 `card-war-made-by-class-3`。

## v0.4.0 - 文章管理后台

- 新增后端管理接口：创建、编辑、删除文章。
- 新增前端“管理”页，支持填写标题、摘要、正文、标签、日期和阅读时长。
- 管理页可编辑已有文章、删除文章，并自动刷新文章列表。
- 整理 Markdown 文件，把阶段性安装/排查/旧流程文档移入 `docs/archive/`。
- 修复小游戏仓库中开始游戏后的界面滚动问题，相关提交已推送到 `card-war-made-by-class-3`。
- 更新 README、版本日志、代码分析文档和下一步计划。

## v0.3.0 - PostgreSQL 数据持久化版本

- 新增 `docker-compose.yml`，通过 Docker Compose 启动 PostgreSQL。
- 新增 SQLAlchemy 数据库层、数据模型和种子数据初始化。
- 后端 `/api/articles`、`/api/articles/{id}/comments`、`/api/articles/{id}/reaction` 改为数据库读写。
- 前端评论、点赞、收藏、点踩改为调用后端接口，数据可持久化。
- 新增 `数据库说明.md`，记录数据库启动、表结构、接口行为和验证命令。
- 更新 README、下一步计划和代码分析文档。

## v0.2.3 - Git 代理和小游戏响应式修正

- 为当前仓库配置 Git 本地代理 `http://127.0.0.1:7897`，通过 Clash 成功恢复 GitHub 推送。
- 将之前本地未推送的 3 个提交推送到 GitHub `main` 分支。
- 调整小游戏嵌入布局，从两列占位改为上方控制条 + 下方全宽 iframe，避免压缩游戏宽度。
- 更新 `GITHUB_PUSH_TROUBLESHOOTING.md` 和 `NEXT_STEPS.md`，记录 GitHub 推送问题已解决。

## v0.2.2 - Docker 与 GitHub 推送准备文档

- 新增 `DOCKER_DESKTOP_SETUP.md`，说明 Docker Desktop、WSL/Ubuntu、Docker Compose 的安装和验证步骤。
- 新增 `GITHUB_PUSH_TROUBLESHOOTING.md`，记录 GitHub 推送失败的排查结果和代理/GitHub Desktop/API 三种替代方案。
- 更新 `NEXT_STEPS.md` 和 README，把 Docker 安装与 GitHub 推送排查作为 v0.3.0 前置事项。

## v0.2.1 - 下一步计划文档

- 新增 `NEXT_STEPS.md`，明确当前状态、下一步推荐方向和 v0.3.0 数据持久化计划。
- 更新 README，修正 GitHub 远程仓库状态，并加入下一步开发入口。
- 更新代码分析文档，记录新增文档的用途。

## v0.2.0 - 小游戏嵌入版

- 将小游戏模块从占位入口改为直接嵌入 GitHub Pages 在线页面。
- 新增“刷新游戏”“新窗口打开”“查看仓库”三个操作入口。
- 后端 `/api/game/card-war` 接口新增 `playUrl`，返回小游戏在线访问地址。
- 更新代码分析文档，记录小游戏集成方式。

## v0.1.0 - 本地 MVP 起步版

- 重写 `需求.md`，将原始需求整理为系统化 PRD 和阶段计划。
- 创建 React 前端项目骨架。
- 创建 FastAPI 后端接口骨架。
- 实现个人主页、文章列表、搜索、互动按钮、AI 模块和小游戏入口的前端雏形。
- 增加 README 和代码分析文档。
- 暂未接入 PostgreSQL、GitHub OAuth、GitHub Actions 和云服务器部署。
