...
当前最新阶段：v2.7.4 首页秒级计时热修版。博客已经完成文章发布、分类归档、Markdown/LaTeX、图片上传、评论回复和审核、账号中心、管理员后台、站点统计、GitHub OAuth、云端部署、自动发布、AI 写作辅助、站点状态展示、高级搜索、登录限流、批量评论审核、后台夜间模式、主页作品集化、移动端导航整理、运维页加载稳定性和首页秒级上线计时闭环。

GitHub 推送问题已通过 Clash `127.0.0.1:7897` 代理解决，排查细节已归档到 `docs/archive/GITHUB_PUSH_TROUBLESHOOTING.md`。

## 48. v2.7.4 已完成：首页秒级计时热修

本阶段已完成：

- 修复首页 `homeNow is not defined` 运行时错误。
- 秒级上线计时状态移动到首页组件内部，避免跨组件作用域读取失败。
- 保留首页上线时间每秒自动刷新。

部署后请重点测试：

1. 打开首页，确认不会再进入“页面刚刚卡住了”。
2. 确认“上线时间”显示到秒，并且停留几秒后会自动跳动。
3. 点进文章、计划、后台再回到首页，确认首页仍然正常。

## 47. v2.7.3 已完成：首页上线时间精确到秒

本阶段已完成：

- 首页状态卡的“上线天数”改为“上线时间”。
- 上线时间显示为“天、小时、分钟、秒”，并每秒自动刷新。
- 状态卡补充长文本换行和手机端字号处理，避免窄屏撑破布局。
- 更新前端版本清单和 `CHANGELOG.md`。

部署后请重点测试：

1. 打开首页，确认“上线时间”显示到秒。
2. 在首页停留 5 秒以上，确认秒数会自动跳动。
3. 手机竖屏打开首页，确认上线时间不会撑破状态卡。
4. 切换日间/夜间模式，确认状态卡文字清晰。

## 46. v2.7.2 已完成：运维页加载稳定性

本阶段已完成：

- 运维面板取消动态分包，改为随主入口一起加载。
- 避免部署后浏览器旧缓存继续请求过期的运维模块文件，导致运维页打不开。
- 保留运维页局部错误保护，真实健康检查或外部接口异常时不影响后台其他页面。
- 更新前端版本清单和 `CHANGELOG.md`。

部署后请重点测试：

1. 管理员登录后进入后台，点击“运维”，确认页面可以直接打开。
2. 手机浏览器刷新后再次进入“运维”，确认不会出现白屏或“页面刚刚卡住了”。
3. 在运维页点击“真实检查”和“刷新部署”，确认接口异常时只显示提示，不把页面整块打崩。
4. 切换到“总览、版本、写文章”，确认后台其他页面仍然正常。

## 45. v2.7.1 已完成：移动端布局整理

本阶段已完成：

- 移动端侧栏从多行大图标区改成单行横向滚动导航，减少顶部占高。
- 横屏移动端让头像品牌区和导航并排显示，给正文释放更多垂直空间。
- 暑期计划表在手机宽度下改为卡片流，字段自带标签，不再强制横向大表格。
- 移动端计划表卡片补齐夜间模式颜色。

部署后请重点测试：

1. 手机竖屏打开首页、文章、计划，确认顶部导航不再占据大块高度。
2. 手机横屏打开计划页，确认头像区和导航在同一行，正文能更早露出来。
3. 打开暑期计划，确认时间段、要做什么、说明、类型以卡片形式展示。
4. 切换夜间模式，确认移动端计划表卡片没有白底或看不清的灰字。
5. 横向滑动顶部导航，确认所有入口仍然可以点到。

## 44. v2.7 已完成：主页故事线和评论体验

本阶段已完成：

- 首页新增学习故事线，把课程笔记、工程闭环、AI 工作流串成更清晰的个人主页叙事。
- 首页新增作品墙，突出个人博客工作台、AI 写作学习助手、运维和备份面板。
- 评论区新增公开评论数、最近互动、我的状态摘要。
- 评论返回中新增作者角色，前端可显示“站长”“我”“待审核”徽章。
- 登录用户可以看到自己待审核的评论，提交后不会像丢失一样从页面消失。

部署后请重点测试：

1. 打开首页，确认故事线、当前探索、作品墙在日间和夜间模式都清晰。
2. 手机宽度下打开首页，确认故事线和作品墙不会横向挤压。
3. 用普通账号评论文章，确认提交后能看到自己的“待审核”评论和提示。
4. 用管理员账号评论文章，确认评论显示“站长”标识，并且可直接公开或按配置进入审核。
5. 游客打开文章，只能看到已通过评论，看不到普通用户待审核评论。

## 43. v2.6.3 已完成：夜间模式文字对比度修复

本阶段已完成：

- 备份中心统计卡的标签和值补充深色主题文字颜色，修复灰字几乎不可见的问题。
- 版本指标、版本勾选项、后台洞察说明文字统一接入 `text-soft` / `text-main` 主题变量。
- 暑期计划里的模块说明、日期选择器等弱文本补齐夜间模式对比度。
- 更新前端版本清单和 `CHANGELOG.md`，当前版本显示为 `v2.6.3`。

部署后请重点测试：

1. 切换夜间模式，打开后台“备份”，确认“备份记录、覆盖范围、最近记录”的标签和值清晰可见。
2. 打开“版本”，确认统计卡、版本条目和勾选改动点都能看清，当前版本显示 `v2.6.3`。
3. 打开“总览、写文章、文章库、图片、评论、安全、群聊语料、学习助手、计划”，快速扫一遍灰色说明文字是否还有看不清的区域。
4. 切回日间模式，确认这些区域没有被改得过重或刺眼。

## 42. v2.6.2 已完成：后台夜间模式补全

本阶段已完成：

- 后台文章库列表卡片、图片资源卡、说明卡和空状态统一纳入深色主题。
- 上传按钮、普通操作按钮、删除按钮和文章状态徽章补充深色模式配色。
- 评论管理、安全页、备份中心、微信群 bot 语料工作台里的浅色块已改为深色面板。
- 版本面板同步更新到 `v2.6.2`，避免只更新文档不更新页面显示。

部署后请重点测试：

1. 切换夜间模式，逐个打开后台“写文章、文章库、图片、评论、安全、备份、群聊语料”。
2. 确认列表卡片、空状态、说明卡和按钮都不再出现大面积白底。
3. 确认删除按钮仍然能看出危险操作语义，但不会在夜间模式中过亮。
4. 打开版本页，确认当前版本显示 `v2.6.2`。

## 41. v2.6.1 已完成：可用性、安全和性能打磨

本阶段已完成：

- 文章中心搜索支持关键词高亮，以及 `tag:`、`category:`、`month:` 高级筛选。
- 首页新增站点状态卡片和最近更新入口，展示上线天数、文章数、阅读数和评论数。
- 管理后台评论页新增“通过当前筛选”，可批量通过当前筛选结果里的待审评论。
- 登录增加前端本地失败锁定和后端 IP + 邮箱维度限流。
- 深色模式和移动端样式继续修补，新增状态卡、最近更新条和搜索高亮在窄屏下更稳定。
- 前端构建拆分 React、图标和 KaTeX 代码块，减少主入口包压力。
- `README.md`、`CHANGELOG.md` 和 `.env.example` 已同步维护。

部署后请重点测试：

1. 手机打开首页、文章页、管理页，确认没有横向溢出、遮挡或低对比度文字。
2. 在文章中心分别测试普通关键词、`tag:AI`、`category:学习笔记`、`month:2026-07`。
3. 管理后台评论页筛选待审评论后点击“通过当前筛选”，确认评论公开且统计刷新。
4. 登录页连续输错密码，确认前端提示和后端限流不会影响正常等待后的登录。
5. 首页站点状态、最近更新、公开文章数和评论数是否符合线上数据。
6. GitHub Actions 自动部署完成后，强制刷新浏览器缓存再复测深色模式。
...
- `.env.example`、`docker-compose.yml`、`本地和服务器启动指南.md` 已补充 Codex 中转配置说明。
...
## 39. v2.1.0 已完成：项目验收收尾版

本阶段已完成：

- 新增后台站点统计面板：阅读量、用户、评论、待审评论、草稿、热门文章和分类分布。
- 新增 AI 设置和测试面板：可查看服务器配置状态，也可用一次性表单测试真实模型。
- AI 写作历史已落库，刷新后仍可查看最近生成记录。
- 文章新增分类和置顶，文章页新增分类筛选。
- 评论新增回复目标和账号级频率限制。
- 新增账号中心，展示当前用户的评论、互动和收藏记录。
- 补充验收演示内容和 API Key 安全说明。

部署后请重点测试：

1. 首页是否展示项目验收阶段。
2. 文章页分类筛选、置顶标记和文章详情是否正常。
3. 登录后账号中心是否显示评论和互动记录。
4. 评论回复是否进入待审，管理员通过后是否公开展示。
5. 后台统计、AI 设置测试、AI 历史是否正常。

下一阶段建议：v2.2.0 做 SEO、站点地图、分享卡片和公开访问体验优化。
...
- `.env.example`、`docker-compose.yml` 和 `本地和服务器启动指南.md` 已补充 `AI_REQUEST_TIMEOUT` 与真实模型配置说明。
...
完整启动、停止和排查步骤见 [本地和服务器启动指南.md](../../guides/本地和服务器启动指南.md)。
...
4. 如果准备部署上线，准备云服务器信息：
   ...
   - 域名，如果已有
5. 购买前先阅读 [云服务器和域名准备.md](../../guides/云服务器和域名准备.md)。
6. 真正开始部署时按 [云服务器部署指南.md](../../guides/云服务器部署指南.md) 执行。
...
- README 增加快速启动和云部署入口。
- 《本地和服务器启动指南》增加本地启动清单和云部署关系说明。
- 新增《云服务器部署指南》，记录云服务器部署步骤。
- 《云服务器和域名准备》增加部署执行入口。
