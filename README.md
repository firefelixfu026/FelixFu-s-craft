...
- 文章列表、文章详情、关键词高亮搜索、标签、分类、归档、置顶和阅读时长
...
- 评论支持分页、字数限制、回复、后台管理、批量审核、删除和管理员免审开关
- 账号中心可查看自己的评论、收藏文章和互动记录
- 首页支持极客风个人展示、日夜间模式、侧边栏折叠、站点状态和最近更新展示
...
- `LOGIN_FAILURE_LIMIT`：同一邮箱和来源连续登录失败多少次后短暂锁定，默认 `5`
- `LOGIN_LOCK_SECONDS`：触发登录锁定后的等待秒数，默认 `60`
...
- 每个账号对同一篇文章的互动只记录一次
- 评论和文章操作都走后端权限校验
- 登录失败会触发短暂限流，降低暴力尝试风险
- API Key 和 OAuth 密钥只保存在服务器环境变量中
- 上传图片会限制类型和访问路径

## 项目性质

这是一个个人学习、技术写作和作品集展示项目。
