...
- 吉祥物现在是可拖动的悬浮小窗，点击会说话，右键可以关闭或切换形态，关闭后能在账号页重新显示。
- 计划页已拆成 `/plan/schedule`、`/plan/completion`、`/plan/courses`、`/plan/apps`、`/plan/finance`、`/plan/meals`、`/plan/body`、`/plan/sleep`。
- 友链独立为 `/links`，工具箱公共页只展示资源，自定义网站在后台“工具箱”里管理。
- 游戏页改为游戏库入口，后续小游戏可以继续挂到同一页面。
- 后台内容库改为文件资源管理器式分组，便于查看文章属于哪个大类、哪个收纳路径和当前顺序。
...
- 版本清单按大版本做成时间线，保留从 `v0.1.0` 到当前版本的完整演进记录。
- 工具箱支持管理员自定义网址，新增的链接会保存到后端数据库并公开展示。
- 首页加入 Live2D 吉祥物“黍泡泡”，模型来源为 Bilibili UP 主切丁鱼片。
...
- 吉祥物
  - 首页右下角显示 Live2D 看板娘“黍泡泡”。
  - 支持换台词、收起和再次叫出，收起偏好保存在当前浏览器。
  - Live2D 运行库只在首页需要时加载，其它页面不会额外加载。

- 互动和账号
  - 支持账号登录后评论、点赞、收藏、点踩和使用“？”反馈。
  - 评论后台目前只保留查看和删除，默认直接公开评论。
  - 账号中心可以查看自己的评论、收藏文章和互动记录。
  - 管理员可以在账号中心上传并更新侧边栏头像。

- 管理后台
  - 写作台用于发布随笔和普通文章，正文编辑和预览左右对齐。
  - 笔记上传用于导入技术笔记。
  - 内容库用于编辑、删除、置顶、批量发布和管理草稿。
  - 音乐管理用于上传和删除音乐。
  - 版本记录用时间线展示大版本阶段和关键小版本。
  - 运维页用于查看服务状态和部署辅助信息。
  - 安全页保留操作日志和删除保护说明。
  - 夜间模式覆盖后台常用卡片、提示条和操作按钮。
  - 视觉巡检页集中展示后台卡片、表单、按钮和提示条，方便切换日夜间模式时快速排查漏白底。
